...
router.get("/crawl_tool_story", async (req, res) => {
  return res.send("craw đasadf=dddddfffcvbfgd successsss");
});

router.post("/crawl_tool_story", async (req, res) => {
  fs.rmSync(path.join(__path_app+'/public/download/', 'table.txt'), { recursive: true, force: true });
  fs.rmSync(path.join(__path_app+'/public/download/', 'time.txt'), { recursive: true, force: true });
  ...
  const downloadTruyenfull = async (link, number) => {
    ...
    try {
      ...
      // Description
      await page.waitForSelector(".col-truyen-main .desc-text.desc-text-full", {
        visible: true,
      });
      ...
      const status = await page.$eval(
        ".col-truyen-main .info-holder .info span",
        (el) => {
          return el.innerText;
        }
      );
      ...
      const dirStoryName =
        __base + "public/download/" + RandomString.changeSlug(titleStory) + "/";
      Functions.createFolderAnfFile(dirStoryName, "title.txt", titleStory);
      Functions.createFolderAnfFile(
        dirStoryName,
        "description.txt",
        description
      );
      ...
      for (const chapter of listChapter) {
        for (let i = 0; i < 2; i++) {
          try {
            ...
            console.log(
              "Lỗi đường link lần " + i + " " + chapter.url + ": " + error
            );
          }
        }
      }
  ...
  // await HandlePage.closeBrowser(browser);
  return res.send("craw story success");
});
